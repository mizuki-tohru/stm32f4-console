...
// 			UART = (USART_TypeDef *) USART6_BASE;
			...
			USART_StructInit(&USART_InitStructure);
			USART_InitStructure.USART_BaudRate = baudrate;
			USART_Init((USART_TypeDef *)USART6_BASE, &USART_InitStructure);
...
//			pUSART_Buf = &USART6_Buf;
			...
			/* Enable USART Receive interrupts */
			USART_ITConfig((USART_TypeDef *)USART6_BASE, USART_IT_RXNE, ENABLE);
			USART_Cmd((USART_TypeDef *)USART6_BASE, ENABLE);
	...
	switch(port) {
	  ...
	  case 6:
		...
		if(TXBuffer_FreeSpace) {
			...
			__enable_irq();
			/* Enable TXE interrupt. */
			USART_ITConfig((USART_TypeDef *) USART6_BASE, USART_IT_TXE, ENABLE);
		}
		return TXBuffer_FreeSpace;
		break;
	}
...
inline void putch_p(uint32_t port,uint8_t data)
{
	...
	switch (port) {
	  ...
	  case 4 :
		...
		break;
	  case 6 :
		while(!USART_TXBuffer_FreeSpace(&USART6_Buf));
		USART_TXBuffer_PutByte(port,&USART6_Buf,data);
		break;
	}
}

/*
extern void putcSD(uint8_t data);
...
uint8_t getch_p(uint32_t port)
{
	switch (port) {
	  ...
	  case 4 :
		...
		break;
	  case 6 :
		if (USART_RXBufferData_Available(&USART6_Buf))
			return USART_RXBuffer_GetByte(&USART6_Buf);
		else
			return false;
		break;
	}
...
uint16_t getch_p16(uint32_t port)
{
	switch (port) {
	  case 1 :
		if (USART_RXBufferData_Available(&USART1_Buf))
...
uint8_t keypressed(uint32_t port)
{
	switch (port) {
	  ...
	  case 6 :
		return (USART_RXBufferData_Available(&USART6_Buf));
	}
	return 0; // ?
